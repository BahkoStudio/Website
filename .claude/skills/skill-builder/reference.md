# Claude Code Skills — Full Technical Reference
...
Skills self-heal. When a skill runs and produces an error, the agent must automatically patch the SKILL.md (and/or its supporting files/scripts) so the same failure cannot recur.
...
- **Wrong model ID errors:** Correct model IDs as of 2025: `claude-opus-4-6`, `claude-sonnet-4-6`, `claude-haiku-4-5-20251001`.
...
`name` defaults to directory name. Everything else is optional.
